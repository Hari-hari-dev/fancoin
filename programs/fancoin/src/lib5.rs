...
pub mod fancoin {
    ...
    pub fn register_validator_pda(
        ...
        let clock = Clock::get()?;
        let current_time = clock.unix_timestamp;
        let current_minute = ((current_time % 3600) / 60) as u32; // Minute of the current hour
        let current_hour = (current_time / 3600) as u32;
    
        // Extract last_reset_hour from the game
        //let last_reset_hour_option = game.last_reset_hour;
    
        // Extract last_activity from the validator_pda
        let validator_last_activity = ctx.accounts.validator_pda.last_activity;
        let validator_last_activity_hour = (validator_last_activity / 3600) as u32;
    
        // Check if validator has punched in this hour
        if validator_last_activity_hour != current_hour {
            msg!("Validator hasn't used punch_in this hour.");
            return Ok(());
        }
        // Check if the game has been reset for the current hour
        if let Some(last_reset_hour) = game.last_reset_hour {
            if current_hour > last_reset_hour {
        ...
        // Block minting during the first 00-07 minutes of each hour
        if current_minute < 7 {
            msg!("Minting is blocked during the first 7 minutes of the hour.");
            return Ok(()); // Exit early without minting
        }
    
        msg!("submit_minting_list => player_ids={:?}", player_ids);
        let total_vals = game.validator_count as usize;
        let active_vals = game.active_validator_count as usize; // Get active validators
        ...
        for pid in player_ids {
            ...
            // (B) Adjusted Minting Condition based on active validators
            ...
            // Mint tokens to the player's ATA
            token::mint_to(
            ...
            for vk in &player_pda.partial_validators {
                // Derive the ATA for the validator
                let (validator_ata, _validator_ata_bump) = Pubkey::find_program_address(
                    &[vk.as_ref(), &token::ID.to_bytes()],
                    &token::ID,
                );
    
                // Fetch the ATA from remaining_accounts
                let ata_account = match leftover_iter.next() {
                    Some(acc_info) => Account::<TokenAccount>::try_from(acc_info)?,
                    None => {
                ...
                // Mint tokens to the validator's ATA
                token::mint_to(
                    CpiContext::new_with_signer(
                        ctx.accounts.token_program.to_account_info(),
                        MintTo {
