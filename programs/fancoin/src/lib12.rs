...
//impl MintAuthority { pub const LEN: usize = 17; }
...
#[derive(Accounts)]
#[instruction(name: String, mint_pubkey: Pubkey)]
pub struct RegisterPlayerPda<'info> {
    ...
    #[account(mut, address = game.mint_pubkey)]
    pub fancy_mint: InterfaceAccount<'info, Mint>,
    // CHECK: We do not read or write this account in the program.
    // It is only provided as a pass-through or for external checks.
...
pub struct CreateUserAtaIfNeeded<'info> {
    ...
    #[account(address = game.mint_pubkey)] // or pass it in as well
    pub fancy_mint: InterfaceAccount<'info, Mint>,
...
pub struct SubmitMintingList<'info> {
    ...
    #[account(
        seeds = [b"mint_authority", game.mint_pubkey.as_ref()],
...
pub mod fancoin {
    ...
    pub fn initialize_game_and_mint(
        ...
        msg!(
            "Created new Game => mintedMint={}, game_pda='{}'",
