...
pub struct Game {
    ...
    /// Whether validators must be “gated” or can freely register.
    pub gated_val: bool,
    pub owner: Pubkey,
...
impl PlayerNamePda {
    ...
    pub const LEN: usize = 8 + (4 + Self::MAX_NAME_LEN) + 32 + 2;  
}
...
pub struct RegisterValidatorGated<'info> {
    #[account(
        mut,
        has_one = owner @ ErrorCode::Unauthorized,
        seeds = [b"game", mint_pubkey.as_ref()],
    ...
    /// Create the new validator’s PDA from seeds = [b"validator", mint_pubkey, validator_to_add]
    #[account(
        init,
        payer = owner,
        space = ValidatorPda::LEN,
    ...
    /// The mint must match game.mint_pubkey
    #[account(
        mut,
    ...
    #[account(
        mut,
        constraint = validator_to_add_acc.key() == validator_to_add
    )]
    pub validator_to_add_acc: Account<'info, ValAdd>,

    /// The new validator’s ATA => anchor auto-creates for `validator_to_add`
    #[account(
        init_if_needed,
        payer = owner,
        associated_token::mint = fancy_mint,
        associated_token::authority = validator_to_add_acc
    )]
    pub validator_ata: InterfaceAccount<'info, TokenAccount>,

    /// If you do not do any Token2022 calls here, you can remove `token_program`.
    /// We'll remove `mint_to` from the import if not used.
    #[account(address = TOKEN_2022_PROGRAM_ID)]
    pub token_program: Program<'info, Token2022>,
...
pub mod fancoin {
    ...
    pub fn initialize_game_and_mint(
        ...
        msg!(
            "Initialized => commission_ata={}, pct={}, issuance={}, val_rate={}, gated={}, owner={}",
            game.commission_ata,
    ...
    pub fn update_commission_info(
        ...
        // 1) Ensure new_commission_percent <= 100
        require!(
            new_commission_percent <= 100,
            ErrorCode::CommissionTooLarge
        );
        ...
        msg!(
            "Commission updated => new_ata={}, new_percent={}%",
            game.commission_ata,
            game.commission_percent
        );
    ...
    pub fn register_validator_gated(
        ctx: Context<RegisterValidatorGated>,
        mint_pubkey: Pubkey,
        validator_to_add: Pubkey,
    ) -> Result<()> {
        let game = &mut ctx.accounts.game;
    
        // If game.gated_val == false, disallow
        require!(game.gated_val, ErrorCode::GameIsNotGated);
        ...
        msg!(
            "Gated validator registration => new validator={}, ATA={}, game={}",
            validator_to_add,
    ...
    pub fn punch_in(ctx: Context<PunchIn>, mint_pubkey: Pubkey) -> Result<()> {
        let clock = Clock::get()?;
    ...
    pub fn register_validator_pda(
        ...
        let game = &mut ctx.accounts.game;
        //require!(game.game_number == game_number, ErrorCode::GameNumberMismatch);
        require!(!game.gated_val, ErrorCode::GameIsGated);
...
pub enum ErrorCode {
    ...
    CommissionTooLarge,
    #[msg("Game is gated, so open registration is disallowed.")]
    GameIsGated,
    #[msg("Game is not gated, so this instruction is disallowed.")]
    GameIsNotGated,
}
