...
// We leave DApp unchanged (no lines removed).
#[account]
pub struct DApp {
    pub owner: Pubkey,
...
// --------------------------------------------------------------------
// *** ADDED: We store mint_pubkey in Game to support multiple games. ***
// We also keep the original fields, including game_number, unchanged.
// --------------------------------------------------------------------
#[account]
pub struct Game {
    // *** The new field for parallel games keyed by mint_pubkey: ***
    pub mint_pubkey: Pubkey, 
    pub gated_val: bool,       // If true, only mint_owner can register validators.
    pub mint_owner: Pubkey,    // The “owner” who can add validators in gated mode.

    // Original fields remain (not removing lines):
    pub game_number: u32,
...
// We updated the LEN accordingly (adding 32 bytes for `mint_pubkey`):
impl Game {
...
pub struct InitializeGame<'info> {
    // Replacing old seeds with new seeds that use `mint_pubkey`.
...
pub struct UpdateGameStatus<'info> {
    // old: seeds = [b"game", &game_number.to_le_bytes()],
    #[account(
        mut,
        seeds = [b"game", mint_pubkey.as_ref()],
        bump
...
pub struct PunchIn<'info> {
    ...
    // old seeds: seeds = [b"validator", &game_number.to_le_bytes(), validator.key().as_ref()]
    #[account(
        mut,
...
pub struct RegisterValidatorPda<'info> {
    // old: seeds = [b"game", &game_number.to_le_bytes()],
    #[account(mut, seeds = [b"game", mint_pubkey.as_ref()], bump)]
    pub game: Account<'info, Game>,

    #[account(mut, address = dapp.mint_pubkey)]
    pub fancy_mint: Account<'info, Mint>,

    // old seeds for validator: seeds = [b"validator", &game_number.to_le_bytes(), user.key().as_ref()]
    #[account(
        init,
        payer = user,
        space = ValidatorPda::LEN,
        seeds = [b"validator", mint_pubkey.as_ref(), user.key().as_ref()],
        bump
    )]
    pub validator_pda: Account<'info, ValidatorPda>,
...
pub mod fancoin {
    ...
    pub fn initialize_game(
        ...
        game.gated_val = gated_val;
        game.mint_owner = ctx.accounts.user.key(); // the signer who calls initialize_game
        // fill the new field
        game.mint_pubkey = mint_pubkey;

        // keep the old "game_number" lines
        ...
        msg!("Game #{} with mint {} initialized => {}", game_number, mint_pubkey, game.description);
        Ok(())
    }
    ...
    pub fn register_validator_pda(
        ...
        game_number: u32,
        mint_pubkey: Pubkey,
        maybe_new_validator: Option<Pubkey>, // if you want the “new address”
    ) -> Result<()> {
        let game = &mut ctx.accounts.game;
         require!(game.game_number == game_number, ErrorCode::GameNumberMismatch);
        require!(game.mint_pubkey == mint_pubkey, ErrorCode::GameMintMismatch);
    
        // If gated_val == true, only game.mint_owner can register
        if game.gated_val {
            require!(
                ctx.accounts.user.key() == game.mint_owner,
                ErrorCode::Unauthorized
            );
    
            // Possibly, the actual “validator address” is supplied as a param:
            let new_addr = maybe_new_validator.ok_or(ErrorCode::InvalidSeeds)?;
            ctx.accounts.validator_pda.address = new_addr;
        } else {
            // Non-gated => the caller themselves is the validator
            ctx.accounts.validator_pda.address = ctx.accounts.user.key();
        }
    
        // Continue as before
        ctx.accounts.validator_pda.last_activity = Clock::get()?.unix_timestamp;
        ctx.accounts.validator_pda.last_minted = None;
        game.validator_count += 1;
    
        msg!(
            "Registered validator => game={}, gated_val={} => validator={}, ATA={}",
            game_number,
            game.gated_val,
            ctx.accounts.validator_pda.address,
...
pub struct ClaimValidatorReward<'info> {
    // old seeds => [b"validator", &game_number.to_le_bytes(), validator.key().as_ref()]
    // new seeds => 
    #[account(
        mut,
        seeds = [b"validator", mint_pubkey.as_ref(), validator.key().as_ref()],
...
pub enum ErrorCode {
    ...
    InvalidNameLength,
    #[msg("Game mint address mismatch.")]
    GameMintMismatch,
}
...
fn calculate_group_id_mod(address: &Pubkey, seed: u64, total_groups: u64) -> Result<u64> {
    ...
    let bytes: [u8; 8] = result[0..8].try_into().map_err(|_| ErrorCode::HashConversionError)?;
