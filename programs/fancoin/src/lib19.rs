...
impl PlayerNamePda {
    ...
    pub const LEN: usize = 8 + (4 + Self::MAX_NAME_LEN + 2) + 32 + 2;  
}
...
pub mod fancoin {
    ...
    pub fn register_player_pda_by_validator<'info>(
        ...
        msg!(
            "Gated registration => name='{}', user_authority={}, new player_pda={}",
            name,
            user_authority,
            player_pda.key()
        );
    ...
    pub fn lock_field(ctx: Context<LockField>, field_name: String) -> Result<()> {
        let game = &mut ctx.accounts.game;
    ...
    pub fn validate_player_pubg_time_slim<'info>(
    ...
    pub fn register_validator_gated(
        ...
        let game = &mut ctx.accounts.game;
        require!(game.curated_val, ErrorCode::GameIsNotGated);
        ...
        msg!(
            "Gated validator registration => new_validator={}, ATA={}, game={}",
            validator_to_add,
    ...
    pub fn register_validator_pda<'info>(
        ctx: Context<'_, '_, 'info, 'info, RegisterValidatorPda<'info>>,
        mint_pubkey: Pubkey
    ) -> Result<()> {
        let game = &mut ctx.accounts.game;
        //require!(game.game_number == game_number, ErrorCode::GameNumberMismatch);
        require!(!game.curated_val, ErrorCode::GameIsGated);
        let leftover = &ctx.remaining_accounts;
        require!(leftover.len() >= 1, ErrorCode::InsufficientLeftoverAccounts);
        let wallet_pda_accinfo = &leftover[0];
        let wallet_pda_data = Account::<WalletPda>::try_from(wallet_pda_accinfo)
