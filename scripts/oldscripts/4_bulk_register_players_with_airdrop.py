...
from solders.rpc.responses import SendTransactionResp  # Added import
...
async def send_lamports(
    ...
    try:
        ...
        if tx_details.value is None:
            print(f"[ERROR] Transaction {signature_obj} details not found.")
            return ""
        
        meta = tx_details.value.transaction.meta
        if meta is None:
            print(f"[ERROR] Transaction {signature_obj} meta not found.")
            return ""
        
        if meta.err is not None:
            print(f"[ERROR] Transaction {signature_obj} failed with error: {meta.err}")
            return ""
        ...
        try:
            ...
            if reward_pubkey_str:
                try:
                    ...
                    print(f"[ERROR] Invalid 'player_info_acc_address' in {json_file}. Using player_pubkey as fallback.")
                    reward_pubkey = player_pubkey  # Fallback
            else:
                reward_pubkey = player_pubkey  # Fallback
