...
async def main():
    ...
    # We will then call `initialize_game` which should use invoke_signed to assert PDA and reassign.
    ...
    #  1) Use invoke_signed to reassign 'game' account from system_program to your program-owned PDA address
    ...
    try:
        tx_sig = await program.rpc["init_game_one"](
            ...
            ctx=Context(
                accounts={
                    "game": staging_pubkey,  # The program will reassign this to the PDA internally
                    "user": user,
