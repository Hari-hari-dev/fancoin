...
async def submit_minting_list_with_leftover(
    ...
    seeds_val = [b"validator", dapp_number.to_bytes(4, "little"), bytes(validator_pubkey)]
...
###############################################################################
# 6) Transaction Displayer Function
###############################################################################
async def display_transaction(tx_signature: str):
    """
    Fetches and displays transaction details for a given transaction signature.

    Args:
        tx_signature (str): The transaction signature to display details for.
    """
    try:
        # Fetch transaction details
        confirmed_tx = await provider.connection.get_transaction(tx_signature, encoding="json")
        
        if confirmed_tx.value and confirmed_tx.value.transaction:
            print(f"\n[INFO] Transaction Signature: {tx_signature}")
            print(f"Slot: {confirmed_tx.value.slot}")
            print(f"Block Time: {confirmed_tx.value.block_time}")
            print(f"Fee: {confirmed_tx.value.transaction.meta.fee} lamports")
            print(f"Error: {confirmed_tx.value.transaction.meta.err}\n")

            print("Instructions:")
            instructions = confirmed_tx.value.transaction.message.instructions
            account_keys = confirmed_tx.value.transaction.message.account_keys
            for idx, instr in enumerate(instructions, start=1):
                program_id = account_keys[instr.program_id_index]
                accounts = [account_keys[i] for i in instr.accounts]
                data = instr.data
            ...
            print(f"[WARN] No details found for transaction signature: {tx_signature}")

    except RPCException as e:
        print(f"[ERROR] Failed to fetch transaction: {e}")
    except Exception as e:
        print(f"[ERROR] Unexpected error while fetching transaction: {e}")
        traceback.print_exc()

###############################################################################
# 7) The main function that merges everything
###############################################################################
async def main():
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description="AnchorPy Solana Transaction Displayer and Main Script")
    parser.add_argument(
        "--tx",
        type=str,
        help="Transaction signature to display details for."
    )
    args = parser.parse_args()
        ...
        global provider, program, program_id, dapp_pda, dapp_pda
        ...
        # 2) Derive PDAs
        dapp_number = 1
        (dapp_pda, bump_dapp) = Pubkey.find_program_address([b"dapp", dapp_number.to_bytes(4, "little")], program_id)
            ...
            while True:
                ...
                for iteration in range(1, 5):  # Iterations 1 through 4
                    ...
                    if iteration < 4:
                        ...
                        if matched_names:
                            await submit_minting_list_with_leftover(dapp_number, matched_names, name_map)
