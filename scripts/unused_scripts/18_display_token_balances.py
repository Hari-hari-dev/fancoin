...
async def fetch_paid_players(program: Program, dapp_pda: Pubkey):
    ...
    # If needed, you can fetch the dapp account here:
...
async def _async_fetch_balances_with_commission():
    ...
    # Read dapp_pda + minted_mint
    if not DAPP_PDA_TXT.exists() or not MINTED_MINT_PDA_TXT.exists():
        print("[ERROR] Missing dapp_pda.txt or minted_mint_pda.txt")
