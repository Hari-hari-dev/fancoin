...
def load_pdkeys_from_files():
    with open("dapp_pda.txt", "r") as f:
        dapp_pda_str = f.read().strip()
    ...
    return (dapp_pda, mint_auth_pda, minted_mint_pda)
async def main():
    try:
        ...
        (dapp_pda, mint_auth_pda, mint_for_dapp_pda) = load_pdkeys_from_files()
        dapp_pda, dapp_bump = Pubkey.find_program_address([b"dapp"], program.program_id)
        print(f"DApp PDA: {dapp_pda}, Bump: {dapp_bump}")
