...
async def main():
    ...
    # 4) Read the dapp + minted_mint from local .txt
    ...
    for file_path in json_files:
        try:
            ...
            #   ctx accounts => 
            #     dapp => seeds=[b"dapp", minted_mint]
            #     player_name_pda => seeds=[b"player_name", dapp.key, name.as_bytes()]
            ...
            (player_name_pda, _) = Pubkey.find_program_address(
                [b"player_name", bytes(dapp_pda), player_name.encode("utf-8")],
                program_id
            )
