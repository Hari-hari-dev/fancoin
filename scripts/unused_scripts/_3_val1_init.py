...
async def initialize_dapp_and_mint(program: Program, client: AsyncClient, description: str, socials: str, commission_percent : str, coin_issuance_rate: str, validator_claim_rate: str, gated_val: str, initial_commission_tokens: str):
    """
...
async def update_player_reward_cooldown(
    program: Program,
    client: AsyncClient,
    dapp_pda: Pubkey,
    mint_pubkey: Pubkey,
    ...
    try:
        tx = await program.rpc["update_player_reward_cooldown"](
            mint_pubkey,  # from #[instruction(name, mint_pubkey)]
            user_ata,
            ctx=Context(
                accounts={
...
async def update_commission_info(
    program: Program,
    client: AsyncClient,
    dapp_pda: Pubkey,
    mint_pubkey: Pubkey,
    ...
    try:
        tx = await program.rpc["update_commission_info"](
            mint_pubkey,  # from #[instruction(name, mint_pubkey)]
            user_ata,
            ctx=Context(
                accounts={
                    "new_commission_ata": commission_ata,
...
async def transfer_ownership(
    ...
    try:
        tx = await program.rpc["transfer_ownership"](
            mint_pubkey,  # from #[instruction(name, mint_pubkey)]
            user_ata,
            ctx=Context(
                accounts={
...
async def update_player_name_cooldown(
    program: Program,
    client: AsyncClient,
    dapp_pda: Pubkey,
    mint_pubkey: Pubkey,
    ...
    try:
        tx = await program.rpc["update_player_name_cooldown"](
            mint_pubkey,  # from #[instruction(name, mint_pubkey)]
            name,
            ctx=Context(
                accounts={
...
async def main():
    ...
    try:
        # 1) Create the Dapp+Mint in one go
        description = "Test Dapp"
        socials = "https://website.com"
        commission_percent = 20
        coin_issuance_rate = 2833333
        validator_claim_rate = 28570
        gated_val = 1
        initial_commission_tokens = 1001000000
        (dapp_pda, mint_auth_pda, minted_mint_pda, commission_percent, coin_issuance_rate, validator_claim_rate, gated_val, initial_commission_tokens) = await initialize_dapp_and_mint(program, client, description)
