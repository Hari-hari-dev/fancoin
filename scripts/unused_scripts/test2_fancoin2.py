...
# SPL token + Associated token program IDs
...
async def initialize_dapp(
    program: Program, client: AsyncClient, dapp_number: int, description: str, dapp_pda: Pubkey
) -> Pubkey:
    """Initialize the Dapp, returning the Dapp PDA."""
...
async def register_validator_pda(
    ...
    dapp_number_bytes = dapp_number.to_bytes(4, "little")
    validator_key_bytes = bytes(validator_kp.pubkey())
    seeds = [b"validator", dapp_number_bytes, validator_key_bytes]
...
async def punch_in(program: Program, dapp_pda: Pubkey, dapp_number: int, validator_kp: Keypair):
    ...
    try:
        tx_sig = await program.rpc["register_player_pda"](
            ...
            ctx=Context(
                accounts={
                    "dapp":         dapp_pda,
                    "fancy_mint":   fancy_mint,  # <--- must match the Rust field name
        ...
        print(f"Registered player => {player_pda}, tx: {tx_sig}")
    except RPCException as e:
        ...
        tx_sig = await program.rpc["submit_minting_list"](
            ...
            ctx=Context(
                accounts={
                    ...
                    "token_program":  SPL_TOKEN_PROGRAM_ID,
                    #
                    # We also pass the associated token program in the accounts,
                    # if the on-chain code's Accounts struct references it:
                    #
                    "rent": Pubkey.from_string("SysvarRent111111111111111111111111111111111"),
                    "associated_token_program": ASSOCIATED_TOKEN_PROGRAM_ID,
                    #
                    # The on-chain code might or might not require system_program again;
                    # if so, we can pass it here as well:
                    #
                    "system_program": SYS_PROGRAM_ID,
                },
                signers=[validator_kp],
...
async def main():
    try:
        ...
        # 3) Initialize a Dapp
        dapp_number = 1
        description = "Minimal Dapp Example"
        dapp_pda = await initialize_dapp(program, client, dapp_number, description, dapp_pda)
        ...
        validator_pda = await register_validator_pda(program, client, dapp_pda, dapp_number, validator_kp)

        # 5) Punch in as validator
        await punch_in(program, dapp_pda, dapp_number, validator_kp)
