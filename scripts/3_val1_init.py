...
async def register_validator_gated(
    program: Program,
    client: AsyncClient,
    game_pda: Pubkey,
    mint_pubkey: Pubkey,
    ...
    """
    Calls register_validator_gated(mint_pubkey, validator_to_add).
    ...
    try:
        tx_sig = await program.rpc["register_validator_gated"](
            mint_pubkey,
            validator_pubkey,
            ctx=Context(
        ...
        print("register_validator_gated => success, Tx:", tx_sig)
        tx_resp = await client.get_transaction(tx_sig, commitment=Confirmed)
        if tx_resp.value and tx_resp.value.transaction.meta:
            print("Transaction logs:")
...
async def main():
    ...
    try:
        ...
        # GATED approach
        validator_pda = await register_validator_gated(
            program,
            client,
            game_pda=game_pda,
