...
async def debug_submit_minting_list_onchain(game_number: int, player_names: list[str]):
    ...
    print(f"[DEBUG] On-chain 'submit_minting_list' for game_number={game_number}, players={player_names}")
    try:
...
async def main():
    try:
        ...
        global provider, program, game_pda, program_id, dapp_pda
        ...
        # We assume game_number=1 is already initialized.
        game_number = 1
        (game_pda, _) = Pubkey.find_program_address(
            [b"game", game_number.to_bytes(4, "little")],
            program_id
        )
        print(f"[DEBUG] Found game_pda: {game_pda}")
        ...
        # ------------- 
        # EARLY DEBUG BITS: Check the "Game" on-chain data for game_number=1
        # -------------
        try:
            game_data = await program.account["Game"].fetch(game_pda)
            print(f"[DEBUG] On-chain Game Data =>")
            print(f"         game_number       = {game_data.game_number}")
            print(f"         validator_count   = {game_data.validator_count}")
            print(f"         status            = {game_data.status}")
            print(f"         description       = {game_data.description}")
            print(f"         last_seed         = {game_data.last_seed}")
            print(f"         last_punch_in_time= {game_data.last_punch_in_time}")
        except:
            print("[DEBUG] Unable to fetch Game data. Possibly not initialized or other issue.")
        ...
        if matched_players:
            ...
            if flattened_players:
                ...
                await debug_submit_minting_list_onchain(game_number, flattened_players)
