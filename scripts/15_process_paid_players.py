...
async def fetch_paid_players(program: Program, game_pda: Pubkey):
    ...
    """
    # Not strictly necessary to read the game, but if you need it:
...
async def main():
    ...
    # 2) Read the PDAs
    if not GAME_PDA_TXT.exists() or not MINTED_MINT_PDA_TXT.exists() or not MINT_AUTH_PDA_TXT.exists():
        print("[ERROR] Missing one of the necessary .txt files: game_pda, minted_mint_pda, mint_auth_pda.")
        return
    ...
    # If your code sets `game.commission_percent > 0`, your on-chain program will handle it.
    ...
    for (player_pda_pubkey, acct) in paid_players:
        ...
        print(f"pending_game_time_ms= {acct.pending_game_time_ms}, last_claim_ts={acct.last_claim_ts}")

        # Derive leftover accounts
        (player_name_pda, _bump) = Pubkey.find_program_address(
            [b"player_name", bytes(game_pda), player_name_str.encode("utf-8")],
            program_id
        )
