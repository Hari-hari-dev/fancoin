...
async def register_player_pda(
    ...
    try:
        ...
        if ata_account_info_resp.value is None:
            ...
            ata_owner = Pubkey(ata_account_info_resp.value.owner)
            if ata_owner != player_pubkey:
                print(f"[ERROR] ATA {reward_pubkey} is owned by {ata_owner}, expected {player_pubkey}.")
            else:
                print(f"[INFO] ATA {reward_pubkey} is correctly owned by {player_pubkey}.")
...
async def main():
    ...
    for json_file in json_files:
        ...
        try:
            ...
            # 7f) Verify ATA Ownership Post-Registration
            try:
                # Fetch ATA account info using Solana RPC
                ata_account_info_resp = await client.get_account_info(
                    reward_pubkey, commitment=Confirmed
                )
                if ata_account_info_resp.value is None:
                    print(f"[ERROR] ATA {reward_pubkey} does not exist for player '{player_name}'.")
                    ...
                    if ata_owner != player_pubkey:
                        print(f"[ERROR] ATA {reward_pubkey} is owned by {ata_owner}, expected {player_pubkey}.")
                        # Handle discrepancy as needed (e.g., log, alert, etc.)
                    else:
                        print(f"[INFO] ATA {reward_pubkey} is correctly owned by {player_pubkey}.")
            except RPCException as e:
                print(f"[ERROR] RPCException while verifying ATA for player '{player_name}': {e}")
                traceback.print_exc()
            except Exception as e:
                print(f"[ERROR] Unexpected error while verifying ATA for player '{player_name}': {e}")
                traceback.print_exc()
