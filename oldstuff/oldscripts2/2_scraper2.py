...
def decode_and_collect_players(players):
    ...
    Returns a dict: { "PlayerName": { ...two_accts... }, ... }
    """
    player_data = {}
    if players:
        for p in players:
...
def save_player_keys_as_json(folder: str, name: str, data: dict):
    """
    Saves a single player's data to a JSON file named <folder>/<name>.json.
    """
    ...
    print(f"Saved {name}.json in {folder}/")
...
def main():
    ...
    # We'll store all unique player names -> keypairs in memory as well
    # to avoid rewriting a player's file multiple times if found again.
    all_player_data = {}
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # partial_data is a dict { "PlayerName": <two_accts_dict> }
                # Merge into all_player_data, skipping duplicates
                for player_name, player_info in partial_data.items():
                    # If a name is seen again, we skip regenerating keys
                    if player_name not in all_player_data:
                        all_player_data[player_name] = player_info
                        # Also save to an individual JSON file
                        save_player_keys_as_json("player_keys", player_name, player_info)
            except Exception as e:
                print(f"Error querying {ip}:{port} - {e}")

    print("\nAll player data has been saved individually in the player_keys/ folder.")
